...
public class ToolGunItem
{
	...
	public static bool TryAdd(Player player)
	{
		...
		{
			new SSGroupHeader("MapEditorReborn", false, null),
			new SSDropdownSetting(new int?(0), "Schematic Name", MapUtils.GetAvailableSchematicNames(), 0, 0, null, byte.MaxValue, true)
		};
		bool flag = true;
		for (int i = 0; i < ServerSpecificSettingsSync.DefinedSettings.Length; i++)
		{
			if (list[0].Label == ServerSpecificSettingsSync.DefinedSettings[i].Label)
			{
				flag = false;
				break;
			}
		...
		ServerSpecificSettingsSync.SendToPlayersConditionally((ReferenceHub r) => Enumerable.Any<ItemBase>(r.inventory.UserInventory.Items.Values, delegate(ItemBase x)
		{
			ToolGunItem toolGunItem;
			return x.IsToolGun(out toolGunItem);
		}));
        return true;
    }
